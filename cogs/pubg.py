...
class PUBGStats(commands.Cog):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, headers=self.headers) as resp:
                    if resp.status == 401:
                        raise Exception("PUBG API Key가 유효하지 않습니다. (.env 확인 필요)")
                    if resp.status == 200:
            ...
            self.logger.error(f"PUBG 시즌 정보 로드 실패: {e}", exc_info=True)
    ...
    def save_tracking(self, platform, nickname, stats_content, mode, is_ranked):
        try:
            ...
            self.logger.error(f"PUBG 트래킹 저장 중 오류: {e}", exc_info=True)

    async def fetch_pubg_data(self, platform, target_nick, mode, is_ranked):
        """API 호출 및 데이터 계산 담당 (치명적 에러는 raise)"""
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(player_url, headers=self.headers) as resp:
                if resp.status == 401: raise Exception("PUBG API 401: Unauthorized (Key 확인)")
                if resp.status == 429: raise Exception("PUBG API 429: Rate Limit Exceeded")
                if resp.status == 404: return None, "player_not_found"
                if resp.status != 200: raise Exception(f"PUBG API Error: {resp.status}")
                
                player_data = await resp.json()
                player_id = player_data['data'][0]['id']

            # 2. 시즌 정보 확인
            if is_ranked and not self.current_season:
            ...
            async with session.get(stats_url, headers=self.headers) as resp:
                if resp.status == 404: return None, "no_data"
                if resp.status != 200: raise Exception(f"PUBG Stats API Error: {resp.status}")
            ...
            game_stats = mode_stats.get(mode, {})
