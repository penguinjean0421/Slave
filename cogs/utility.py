...
class Utility(commands.Cog):
    ...
    def load_utility_data(self):
        """데이터 파일을 로드합니다. 실패 시 로그를 남깁니다."""
        try:
            if not os.path.exists(self.data_file):
                raise FileNotFoundError(f"'{self.data_file}' 파일을 찾을 수 없습니다.")
            ...
            self.logger.error(f"Utility 데이터 로드 실패: {e}", exc_info=True)
            self.monsters = ["Zombie"]
            self.death_messages = ["살해당했습니다."]
            self.menu_list = {}
            self.time_data = {}
